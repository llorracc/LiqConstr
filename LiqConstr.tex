...
\begin{verbatimwrite}{./Highlighted/MainMessage} %% Private
...
\end{verbatimwrite}\ifVerbatimWrite{\input{./Highlights/MainMessage.tex}}{} %% Private
...
\begin{verbatimwrite}{./Sections/Setup} %% Private
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}  \label{eq:recursiveV}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}  \label{eq:subphi}
...
% \bibliography{LiqConstr,}\end{document}
...
\begin{verbatimwrite}{./Sections/LCandPS} %% Private
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
    ...
    \\ \hat{V}'({m})  & = \Ex[\hat{V}'({m} + \zeta + \hat{\kappa})] \notag.
  \end{aligned}\end{gathered}\end{equation}
...
\begin{verbatimwrite}{./Sections/Proofs} %% Private
  ...
  \begin{proof}~~First, to facilitate readability of the proof, we assume that $R = \beta = 1$ with no loss of generality. Our goal is to prove that $V({m}_{t}) \in CC$ if $V_{t+1}({a}_{t} + {y}_{t+1}) \in CC$ for all realizations of ${y}_{t+1}$. The proof proceeds in two steps. First, we show that property CC is preserved through the expectation operator (vertical aggregation), i.e.\ that $\Omega({a}_{t}) = \Ex_t[V_{t+1}({a}_{t} + {y}_{t+1})] \in CC$ if $V_{t+1}({a}_{t} + {y}_{t+1}) \in CC$ for all realizations of ${y}_{t+1}$. Second, we show that property CC is preserved through the value function operator (horizontal aggregation), i.e.\ that $V({m}_{t}) = \max_{s} u({c}_{t}({m}_{t} - s)) + \Omega(s) \in CC$ if $\Omega(s) \in CC$. Throughout the proof, the first order condition holds with equality since no liquidity constraint applies at the end of period $t$. 
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\left\{\Ex_t\left[c_{t+1}({a}_{t} + {y}_{t+1})^{-\frac{1}{a}}\right]\right\}^{-a} \geq \left\{\Ex_t\left[\left\{pc_{t+1}({a}_1 + {y}_{t+1}) + (1-p) c_{t+1}({a}_2 + {y}_{t+1})\right\}^{-\frac{1}{\alpha_1}}\right]\right\}^{-\alpha_1}\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\left\{\Ex[(u + v)^{k}]\right\}^{1/k} \geq \left\{\Ex[u^{k}]\right\}^{1/k} + \left\{\Ex[v^{k}]\right\}^{1/k}.\end{aligned}\end{gathered}\end{equation}
    %	
    %	
    %	\begin{equation}\begin{gathered}\begin{aligned}\left\{\Ex[(u + v)^{-k}]\right\}^{-1/k} \geq \left\{\Ex[u^{-k}]\right\}^{-1/k} + \left\{\Ex[v^{-k}]\right\}^{-1/k}.\end{aligned}\end{gathered}\end{equation}
    This implies that for $\alpha_1  \in (0,\infty)$ (CRRA)
    \begin{equation}\begin{gathered}\begin{aligned}\left\{\Ex[(u + v)^{-\frac{1}{\alpha_1 }}]\right\}^{-\alpha_1 } \geq \left\{\Ex[u^{-\frac{1}{\alpha_1 }}]\right\}^{-\alpha_1 } + \left\{\Ex[v^{-\frac{1}{\alpha_1 }}]\right\}^{-\alpha_1 }\end{aligned}\end{gathered}\end{equation}
    ...
    \end{align*}
    which implies that 
    \begin{equation}\begin{gathered}\begin{aligned} (\Omega'({a}_{t}))^{-\alpha_1 } \geq p(\Omega'({a}_1))^{-\alpha_1 } + (1-p)(\Omega'({a}_2))^{-\alpha_1 }\end{aligned}\end{gathered}\end{equation}	
    Thus, defining
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\Omega_t'({a}_{t}) = u'(\chi_t({a}_{t}))\end{aligned}\end{gathered}\end{equation}
  ...
  \begin{proof}
    ...
    \begin{equation}\begin{gathered}\begin{aligned} V'({m}) = u'(c({m}))\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}V'''({m}) = u'''(c({m}))[c'({m})]^2 + u''(c({m}))c''({m})\end{aligned}\end{gathered}\end{equation}
    Absolute prudence of the value function is thus defined as
    \begin{equation}\begin{gathered}\begin{aligned}-\frac{V'''({m})}{V''({m})} &= -\frac{u'''(c({m}))[c'({m})]^2 + u''(c({m}))c''({m})}{u''(c({m}))c'({m})} \nonumber \\
      -\frac{V'''({m})}{V''({m})} &= -\frac{u'''(c({m}))}{u''(c({m}))}c'({m}) - \frac{c''({m})}{c'({m})}\label{eq:absprudence}\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\frac{\hat{V}''({m})}{V''({m})} \equiv 
      \left(\frac{u''(\hat c({m}))}{u''(c({m}))} \right)
      \left(\frac{\hat{c}'({m})}{c'({m})}\right)\end{aligned}\end{gathered}\end{equation}
  ...
  \begin{proof}
    ...
    If $u''' > 0$, a counterclockwise concavification around ${m}^{\#}$ implies that $\hat{c}({m}) < c({m})$ and $\hat{c}'({m}) > c'({m})$ for all ${m} < {m}^{\#}$. Then \begin{equation}\begin{gathered}\begin{aligned}-\frac{u'''(\hat{c}({m}))}{u''(\hat{c}({m})}\hat{c}'({m}) > -\frac{u'''({c}({m}))}{u''({c}({m}))}{c}'({m}) \text{ for } {m} < {m}^{\#} \end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}-\frac{\hat{c}''({m})}{\hat{c}'({m})} \geq -\frac{{c}''({m})}{{c}'({m})} \text{ for } {m} < {m}^{\#}\end{aligned}\end{gathered}\end{equation}
  ...
  \begin{proof}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\frac{u^{\prime\prime}(c_{t})}{\beta R u^{\prime\prime}(c_{t+1})R} \geq  \frac{u^{\prime\prime}(c_{t+1})}{\beta R u^{\prime\prime}(c_{t+1})R} = \frac{1}{\beta R R} > \frac{1}{R}\end{aligned}\end{gathered}\end{equation}
    Furthermore, we know that
    \begin{equation}\begin{gathered}\begin{aligned}c_{t}^{\prime} \geq \frac{R-1}{R}\end{aligned}\end{gathered}\end{equation}
    since $\frac{R-1}{R}$ is the MPC for an infinitely-lived agent with $\beta R = 1$. Hence, 
    \begin{equation}\begin{gathered}\begin{aligned}\frac{c_{t+1}^{\prime}}{c_{t}^{\prime}} = \left(\frac{u^{\prime\prime}(c_{t})}{\beta R u^{\prime\prime}(c_{t+1})R} + c_{t}^{\prime} \right) > \frac{1}{R} + \frac{R-1}{R} = 1\end{aligned}\end{gathered}\end{equation}
  ...
  \begin{proof}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}
      c_{t,n}({{m}})  & = \tilde{c}_{t,n}({{m}}+\kappa_{t,n}) \notag
      \\ c_{t,n+1}({{m}})  & = \tilde{c}_{t,n+1}({{m}}+\kappa_{t,n+1}). \label{eq:hateqtildehat}\end{aligned}\end{gathered}\end{equation}
    ...
    We can rewrite the last (resp. the first) equation of \eqref{eq:hateqtildehat} as
    \begin{equation*}\begin{gathered}\begin{aligned}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}
      ...
      \\  {c}(\kappa_{t,n+1}+\zeta)  & = c_{t+1,n+1}({s}_{t,n+1}+{y}+\kappa_{t,n+1}+\zeta)\notag.
    \end{aligned}\end{gathered}\end{equation}	
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
    ...
    \\   u'({m}-\tilde{s}_{T-1,0}({m}))  & = p u'(\tilde{s}_{T-1,0}({m}))+(1-p) u'(\tilde{s}_{T-1,0}({m})+1). \notag
  \end{aligned}\end{gathered}\end{equation}

  Consider first the case where ${m}$ is large enough that the constraint does not bind for the constrained consumer, ${m}>1$.  In this case the limit of the Euler equation for the second consumer is identical to the Euler equation for the first consumer (because for ${m}>1$ savings are positive for the consumer facing the risk, implying that the limit of the first $u'$ term on the RHS of the second line of \eqref{eq:euler1} is finite). Thus the limit of the second equation in \eqref{eq:euler1} is the first equation in \eqref{eq:euler1} for ${m}>1$.
  ...
  \begin{proof}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\mathcal{P}^{0}_{T,1} = \emptyset \end{aligned}\end{gathered}\end{equation} %%\begin{equation}\begin{gathered}\begin{aligned}\left\{ w| {m} < \omega_{T,1}  \right\}\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\{{m}_{T-1} | \wAlt_{T-1,1} \in \left[{m}_{T-1} - c_{T-1,1}^1({m}_{T-1}) + {y}_{T} + \underline{\zeta} ,{m}_{T-1} - c_{T-1,1}^1({m}_{T-1}) + {y}_{T} + \bar{\zeta}\right] \}\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\mathcal{P}_{T-1,2}^1 = \{{m}_{T-1}| {m}_{T-1} \leq \bar{\wAlt}_{T-1,1}^1 \wedge c_{T-1,2}^1({m}) > {m}_{T-1} \}.\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\mathcal{P}_{T-2,2}^2 = \{{m}_{T-2} | {m}_{T-2} - c_{T-2,2}^2({m}_{T-2}) + {y}_{T-1} + \zeta_{T-1} \in \mathcal{P}_{T-1,2}^1 \wedge c_{T-2,2}^{2}({m}) < {m} \}.\end{aligned}\end{gathered}\end{equation}
    ...
    \begin{equation}\begin{gathered}\begin{aligned}\mathcal{P}_{t,n}^{m-1} = \{{m}_{t} | {m}_{t} \leq \bar{\wAlt}_{t,1}^{m-1} \wedge c_{t,n}^{m-1}({m}) > {m} \}\end{aligned}\end{gathered}\end{equation}
