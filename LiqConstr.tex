  ...
  \parbox{\textwidth}{
    \begin{center}
      \begin{tabbing}
        ...
        \texttt{Dashboard:~} \= \= \url{https://econ-ark.org/materials/LiqConstr?dashboard} \\
        \texttt{~~~~~~PDF:~} \> \> \url{https://llorracc.github.io/LiqConstr/LiqConstr.pdf} \\
        \texttt{~~~Slides:~} \> \> \url{https://llorracc.github.io/LiqConstr/LiqConstr-Slides.pdf} \\
        \texttt{~~~~~html:~} \> \> \url{https://llorracc.github.io/LiqConstr/LiqConstr/}    \\
        \texttt{~~~bibtex:~} \> \> \url{https://llorracc.github.io/LiqConstr/LiqConstr-Self.bib}  \\
        \texttt{~~~GitHub:~} \> \> \url{https://github.com/llorracc/LiqConstr} \\
...
\begin{verbatimwrite}{Sections/Setup} %%% Private
  ...
  \begin{eqnarray*}
    ...
    s_t &=& w_{t} - c_t \\
    s_t &\geq& \sConst_t%%% Private CDC20190605 moved R after w-c because it is increasingly common for R to be specified as a function of (w-c) and we want to be clear that we are NOT doing that 
               \nonumber
  \end{eqnarray*}
...
\begin{verbatimwrite}{Sections/PrudAndCC-Intro} %%% Private
  This section provides a set of tools necessary to prove our main results. We first define what we mean by consumption concavity. Next, we define an operation we call a `counterclockwise concavification,' which describes how either a liquidity constraint or a risk affects the consumption function. The advantage of defining a counterclockwise concavification in such general terms is that we can show that it heightens prudence of the value function irrespective of the source of concavification. Since the relationship between prudence and precautionary saving has already been established in the literature \citep{kimball:smallandlarge}, the tools in this section allows us to later establish how liquidity constraints affect precautionary saving, via consumption concavity and prudence. 
\end{verbatimwrite}
...
\begin{verbatimwrite}{Sections/PrudAndCC} %%% Private
  ...
  This section first defines what we mean by consumption concavity. Next, we show that consumption concavity is a property that recursively propagates, meaning that once established in some period, it also holds in all prior periods under fairly general conditions. Lastly, we define what we mean by greater consumption concavity, which allows us to compare consumption functions. 

  We start by defining what we man when we say that the value function has property local  consumption concavity (CC). 
...
\begin{verbatimwrite}{Sections/LCandCC-Intro} %%% Private
  This section shows under which conditions liquidity constraints cause consumption concavity. The main conceptual issue with liquidity constraints is that the effects of constraints depend on already existing constraints. A constraint introduces a kink in the consumption function at the wealth level where the constraint becomes binding. The introduction of an additional constraint may move or even remove already existing kinks in the consumption function, potentially eliminate local consumption concavity. To get around this issue, we introduce the concept of an \textit{ordered} set of \textit{relevant} constraints. This allows us to add additional constraints in a way that does affect existing kinks in the consumption function. Our main result (Theorem \ref{thm:lcip2}) is that the introduction of the next constraint from the ordered set of relevant constraints causes a counterclockwise concavification of the consumption function. In other words, liquidity constraints cause consumption concavity.  %It follows immediately from Lemma \ref{lem:CCToPrud} and \ref{lem:ccandstrictprud} that liquidity constraints also heighten prudence. 
\end{verbatimwrite}
...
\begin{verbatimwrite}{Sections/LCandCC} %%% Private
  ...
  Recall that we are working with a finite horizon household whose horizon goes from $0$ to $T$. We define a liquidity constraint dated $t$ as a constraint that requires savings at the \textit{end of period} $t \in (0, T]$ to be non-negative. The assumption of non-negativity is without loss of generality since we show in Theorem \ref{thm:lcip2} that our results also hold with general constraints. 
  ...
  \begin{theorem}\label{thm:lcip2} (Liquidity Constraints Cause Counterclockwise Concavification.) \\
    ...
    %	When $n \leq N-1$ constraints have been imposed, the imposition of constraint $n+1$ strictly increases absolute prudence of the agent's value function if the utility function satisfies $u''' > 0$ and $w_t < \wAlt_{t,n+1}$ or if $u''' = 0$ and $\frac{c'_{t,n+1}}{c'_{t,n}}$ strictly declines at $w$.
  \end{theorem}
  ...
  %   Consider an agent who has a utility function with $u'> 0$ and $u'' < 0$, faces constant income ${y}$, and is impatient. Assume that the agent faces a set $\mathcal{T}$ of $N$  constraints. When $n \leq N$ constraints have been imposed, $c_{t,n}(w)$ is a piecewise linear increasing concave function with kink points at successively larger values of wealth at which future constraints stop impinging on current consumption.
  % \end{corollary}
...
\begin{verbatimwrite}{Sections/LCandPS-Intro} %%% Private
...
\centerline{\LARGE \textbf{Appendices}} \medskip
...
\begin{verbatimwrite}{Sections/Proofs} %%% Private
  ...
  \begin{proof}
    ...
    \begin{equation}\label{eq:HARAmu}u'(c) = \begin{cases} \text{ CARA } & \left(ac + b\right)^{-1/a} \text{~for~}  a \in (0,\infty) \\
	\text{ CARA} & e^{-c/b} \text{~for~} a = 0 \\
        \text{ Quadratic} & ac + b \text{~for~}  a = -1\end{cases} \end{equation}
