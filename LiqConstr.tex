...
\parbox{\textwidth}{
  \begin{center}
\begin{tabbing}
  \texttt{Econ-ARK:~} \= \= \url{http://econ.jhu.edu/people/ccarroll/papers/LiqConstr.pdf} \kill \\  
...
\begin{defn}\label{defn:IntervalStrictCC} (Local Consumption Concavity.) \\ In relation to a
	utility function $u(c)$ with non-negative, non-increasing prudence, a function $V(w)$ has property CC (alternately, strict CC) over the
...
To understand the effects on prudence of a counterclockwise concavification, note that for a twice differentiable consumption function and thrice differentiable utility function, absolute prudence is defined as
$$ -\frac{V'''(w)}{V''(w)} = - \frac{u'''(c(w))}{u''(c(w))}c'(w) - \frac{c''(w)}{c'(w)}$$
